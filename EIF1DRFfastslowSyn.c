...
Psyn=mxGetPr(mxTmp);
m1=mxGetN(mxTmp); // col of Psyn
m2=mxGetM(mxTmp); // row of Psyn
if(m1!=Nsyntype)
...
Irecord=mxGetPr(mxTmp);     //记录current的数组
Nrecord = mxGetN(mxTmp);    //记录的个数
m2 = mxGetM(mxTmp);
if(m2!=1)
...
for(jj=0;jj<Nrecord;jj++){  //Nrecord: 记录的I的数量
  if(Irecord[jj]<1 || Irecord[jj]>N+1) //Irecord: 记录要记录的ID
     mexErrMsgTxt("Indices in Irecord must be between 1 and N");
    for(isyn=0;isyn<Nsyn;isyn++){   //记录每个I在Nsyn个syn下的I
        Isynrecord[isyn+jj*Nrecord]=Isyn[(int)round(Irecord[jj]-1)*Nsyn+isyn]; // Isyn: Nsyn*N
    }
  vr[jj]=v[(int)round(Irecord[jj]-1)];
}

/* Refractory states */
Ntref[0]=(int)round(tref[0]/dt); // tref: \tau ref, parameter in EIF model
...
for(i=1; i < Nt && ns < maxns; i++){    //ns: # of spike
    ...
    for(jj=0;jj<N*Nsyn;jj++){   //N = 50000, Nsyn = # of syn type
          ...
          // temp: Nsyn长的数组, Isynprime是Isyn的导数
     ...
     while(*iXspkInd <= i*dt && iXspike < Nsx){
         // even index: spk times. odd index: neuron ID, ++后指向ID
         iXspkInd++; /* point to neuron ID */
         jspike=(int)round((*iXspkInd)-1); // 指向发出这个I的neuron
         if(jspike < 0 || jspike >= Nx){
             mexPrintf("\n %d %d %d %d %d\n",(int)round(*(iXspkInd-1)/dt),iXspike,i,jspike,(int)round((*iXspkInd)-1));
             mexErrMsgTxt("Out of bounds index in sx.");
         }
         Pr = &Wrf[jspike*Kx]; //这个neuron的第一个postsyn neuron
         
         for(k=0;k<Kx;k++){ //遍历这个neuron的所有postsyn neuron, 记录在postcellX中
    ...
    /* loop over neurons  */ 
    Pr=&Wrr[0]; //Wrr: connectivity matrix
    for(j=0;j<N;j++){    
      ...
      /* Spikes will be propagated at the END of the time bin (see below)*/
        ...
        if(j<Ne){
             ...
             if(refstate[j]<=0){
                ...
                for(isyn=0;isyn<Nsyn;isyn++){
                    // Psyn: percentage of different synapse
             ...
             if(refstate[j]<=0){
              ...
              if(v[j]>=Vth[1] && refstate[j]<=0 && ns<maxns){                                                            
                  ...
                  // different form spike occurs in exc neurons
                  // point to the first postsyn neuron of the first inh neuron
